...
App 打 API 的時候，你在 App 畫面上只看得到結果——「載入成功」或「發生錯誤」。光看畫面，你永遠不知道 App 到底送出了什麼、server 究竟回了什麼。測試層面上，這個資訊落差讓大量 bug 變成「無法重現」或「無法定位根因」。

畫面背後，App 發了什麼 request、帶了哪些 header、server 回了什麼 response，你看不到。測試的時候，這些資訊非常重要：
...
QA 用 Proxyman 不是在做壞事，是在用工程師一樣的工具，看清楚 App 和 server 之間真正在說什麼。OWASP Mobile Security Testing Guide 也明確指出，HTTP 流量攔截是 Mobile App 安全測試與功能驗證的基礎能力——QA 掌握這個工具，就是在縮短自己和開發者之間的資訊落差。

---

## 參考資料

- [Proxyman Official Documentation](https://docs.proxyman.io/)
- [OWASP Mobile Security Testing Guide](https://owasp.org/www-project-mobile-security-testing-guide/)
- [Android Network Security Configuration — Android Developers](https://developer.android.com/privacy-and-security/security-config)
- [TLS/SSL Overview — Apple Developer](https://developer.apple.com/documentation/security/preventing-insecure-network-connections)
