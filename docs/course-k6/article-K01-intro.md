...
效能測試的觀念系列裡我們聊過工具選型——這個延伸系列，就專心把「怎麼用 k6 真的做出來」走一遍。先從為什麼選 k6、怎麼裝、第一個腳本開始。
...
- `sleep(1)` 是思考時間——別省，沒有它會高估壓力、低估可承載 VU（這點觀念系列的腳本設計篇講過）。
...
- `http_req_duration`：回應時間。**看 p(95)/p(99)，不要只看 avg**（為什麼，觀念系列指標篇講過）。
