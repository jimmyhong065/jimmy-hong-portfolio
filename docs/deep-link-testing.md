...
Branch.io 的行動增長研究顯示，帶有 Deep Link 的推播通知，比跳轉到首頁的通知有更高的用戶留存率——準確的導航是用戶體驗的一部分。然而，Deep Link 失效是 App 開發中最常被低估的 bug 類型，因為它只在特定的 App 狀態組合下才會出現，而測試時幾乎不會同時考慮所有組合。
...
- [Apple：Supporting Universal Links in Your App](https://developer.apple.com/documentation/xcode/supporting-universal-links-in-your-app) — iOS Universal Link 官方設定與測試說明
- [Android：App Links 驗證](https://developer.android.com/training/app-links/verify-android-applinks) — Android App Link 的設定與驗證流程
- [Apple：在 didFinishLaunchingWithOptions 處理 URL](https://developer.apple.com/documentation/uikit/uiapplicationdelegate/1622921-application) — Cold Start 接收 Deep Link 的正確處理方式
- [Google Testing Blog](https://testing.googleblog.com/) — Google 工程師的測試實踐文章集
- [Firebase Dynamic Links 文件](https://firebase.google.com/docs/dynamic-links) — Firebase 跨平台 Deep Link 解決方案
