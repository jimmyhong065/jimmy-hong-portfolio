# SWE at Google 新創文章 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 寫一篇約 3000 字的繁體中文部落格文章，對比 Google 工程實踐 vs 新創現狀，並上傳到 Supabase 發布。

**Architecture:** 單一 markdown 文章檔案，透過 upload-articles.mjs 上傳至 Supabase，發布到部落格。

**Tech Stack:** Markdown、Node.js upload-articles.mjs、Supabase

---

### Task 1: 建立文章 markdown 檔案

**Files:**
- Create: `docs/swe-at-google-startup.md`

- [ ] **Step 1: 建立文章內容**

建立 `docs/swe-at-google-startup.md`，完整內容如下：
...
```bash
ls -la docs/swe-at-google-startup.md
```

Expected: 檔案存在，大小約 8-10KB

- [ ] **Step 3: Commit**

```bash
git add docs/swe-at-google-startup.md
git commit -m "feat: add SWE at Google startup article draft"
```

---

### Task 2: 上傳到 Supabase 並發布

**Files:**
- Modify: `upload-articles.mjs`（本地，不進 git）

- [ ] **Step 1: 確認 upload-articles.mjs 的 articles 陣列格式**

```bash
grep -n "published" upload-articles.mjs | head -20
```

找到 articles 陣列，確認已發布文章的格式（帶 `published: true`）。

- [ ] **Step 2: 在 upload-articles.mjs 加入新文章（已發布）**

在 articles 陣列加入：

```js
{ filePath: 'docs/swe-at-google-startup.md', published: true },
```

- [ ] **Step 3: 上傳文章**

```bash
ADMIN_PASS=@Pwd0917365 node upload-articles.mjs
```

Expected 輸出：
```
✅ 上傳成功: swe-at-google-startup
```

- [ ] **Step 4: 在部落格確認文章出現**

開瀏覽器，進部落格文章列表，確認：
- 文章標題「《Software Engineering at Google》讀完，我才知道新創少了什麼」出現
- Tags 顯示正確
- 點入文章，目錄正常、三節都在、結尾完整
...
- [ ] `docs/swe-at-google-startup.md` 存在
- [ ] Supabase 有此文章，`published: true`
- [ ] 部落格可正常瀏覽、目錄可點選、無破版
