...
每次 OS 升版、UI 重構、元素 ID 調整，accessibility tree 就可能變。你的測試跟著壞。Capgemini《World Quality Report 2024–25》指出，測試維護成本（尤其是因 UI 改版導致的腳本修復）已成為自動化投資回報率最大的侵蝕來源，部分團隊超過 40% 的自動化工時花在修壞掉的測試，而不是發現新的 bug。
