...
根據 Postman 每年發布的《State of the API》報告，超過 86% 的開發者和測試人員在工作中使用 API 測試工具，但其中有相當大比例仍停留在 GUI 工具的手動操作階段，沒有真正做到自動化。我就是其中一員——用 Postman 手動管理 API 測試，整整兩年。
...
Postman 有 Newman（command line runner），可以在 CI 跑 collection。但 collection 是 JSON 格式，在 git 裡 diff 很難看，review 改動很痛苦。根據 DORA 2024 年的研究，持續整合（CI）是高效能軟體交付團隊最一致的工程實踐之一——而「測試無法穩定放進 CI」是最常見的阻礙，也是讓 API 測試從手動變成真正自動化的關鍵一步。
...
- [Postman State of the API Report](https://www.postman.com/state-of-api/) — 每年發布的 API 使用現況調查
- [pytest 官方文件](https://docs.pytest.org/en/stable/) — Python 測試框架完整文件
- [requests 官方文件](https://requests.readthedocs.io/en/latest/) — Python HTTP 客戶端
- [Martin Fowler：Test Pyramid](https://martinfowler.com/bliki/TestPyramid.html) — 測試金字塔概念，API 測試在其中的定位
- [DORA 2024 研究報告](https://dora.dev/research/2024/dora-report/) — DevOps 效能研究，CI/CD 整合與測試實踐
