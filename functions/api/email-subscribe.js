...
export async function onRequestPost({ request, env }) {
  ...
  // Upsert confirmed=true directly
  ...
  const unsubUrl = `${env.SITE_URL}/api/email-unsubscribe?token=${subscriber.token}`
    ...
    body: JSON.stringify({
      ...
      htmlContent: `
        <div style="font-family:-apple-system,sans-serif;max-width:480px;margin:0 auto;padding:40px 32px;color:#222;">
          ...
          <h1 style="font-size:18px;font-weight:700;margin:0 0 12px;">訂閱成功！</h1>
          <p style="font-size:14px;color:#555;line-height:1.7;margin:0 0 28px;">
            感謝訂閱 Jimmy Hong 部落格，有新文章發布時將會通知您。
          </p>
          <a href="${env.SITE_URL}/blog"
          ...
          <p style="font-size:11px;color:#bbb;margin:32px 0 0;">
